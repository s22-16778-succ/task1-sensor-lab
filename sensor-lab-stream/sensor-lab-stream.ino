...
void printMenu(){
  ...
  Serial.println(" s: Toggle 100 measurement streaming.");
...
void flagSwitch(char c) {
  switch (c) {
    case 'u':
      if(stream == true){
        Serial.println("100 Ultrasonic Distance Sensor measurements:");
        for(int i=0; i<100; i++){
          readUDS();
        }
        Serial.println("100 measurements completed.");
      }else{
        readUDS();
      }
      break;
    case 'p':
      if(stream == true){
        Serial.println("100 Potentiometer measurements:");
        for(int i=0; i<100; i++){
          readPot();
        }
        Serial.println("100 measurements completed.");
    ...
    case 'i':
      if(stream == true){
        Serial.println("100 IR Distance Sensor measurements:");
        for(int i=0; i<100; i++){
          readIR();
        }
        Serial.println("100 measurements completed.");
...
// Ultrasonic Distance Sensor function: Prints ultrasonic distance sensor measurement (in cm).
void readUDS() {
...
// Rotary Potentiometer function: Prints potentiometer output voltage (in V) and angle (in degrees).
void readPot() {
...
// IR Proximity Sensor function: Prints infrared distance sensor measurement (in cm).
void readIR(){
...
void toggleStream(){
  if (stream == false){
    Serial.println("100 measurement stream ON");
    stream = true;
  }else{
    Serial.println("100 measurement stream OFF");
    stream = false;
  }
}
